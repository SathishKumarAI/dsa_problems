...
| What one page still owes, and the ledger of all 153 | `src/lib/page-debt.ts` (the rules) · `docs/PAGE-BACKLOG.md` (generated by `scripts/page-backlog.mjs`) |
...
| **The one page per problem** — `#/p/<pattern>/<id>`, and there is no second route | `src/components/problem-detail.tsx` (the page, its state and the review bands, 490 lines) · **`src/components/problem/`** — one file per zone, map in its `README.md`: `page-sections.ts` (which bands exist and what the rail may offer — plain `.ts`, gated), `orient-zone.tsx`, `act-zone.tsx`, `compare-view.tsx`, `closing-bands.tsx` · `problem-statement.tsx` (statement, bounds, examples) · `approach-ladder.tsx` (the rungs) · `problem-closing.tsx` (reading list + the right rail) |
