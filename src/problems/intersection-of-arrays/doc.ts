// intersection-of-arrays — the teaching document, as data.
//
// Converted from docs/deep/intersection-of-arrays_explained.md by
...
export const doc: TeachingDoc = {
  ...
  understanding: `You are handed two integer arrays. Return the values they have in common — but with a twist that is
...
---`,
  ...
  arc: `The principle every rung chases is *stop re-deriving what you already know*, and here the thing being
...
---`,
  ...
  interview: `**Know cold: count the smaller side, and the two-cursor merge.** They are a matched pair, and the
...
---`,
  scriptNote: `Every approach in one file, checked against both examples from the statement, the smallest legal
...
arrays. Every answer is **sorted before comparison**, because the judge accepts any order.`,
  script: `"""What Both Arrays Hold — every approach in one file, cross-checked.
