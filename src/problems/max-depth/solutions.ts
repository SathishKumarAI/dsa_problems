...
export const costWhy =
  "Every node is visited exactly once and does a constant amount of work \u2014 two recursive calls, a maximum, an addition \u2014 so the time is O(n) and cannot be less: the depth is not knowable without seeing every node. The space is the call [[stack]], which holds one frame per level on the current path: O(h), where h is the height. That is the bound worth reading carefully, because h is log n on a balanced tree and n on a chain, and the constraints allow a 10\u2074-node chain \u2014 which is past CPython\u2019s default recursion limit of 1000. Same asymptotic class as the iterative rungs, and a crash on legal input. It is also SLOWER than the BFS rung — 2.3 ms against 1.8 on a complete tree of 10⁴ nodes, best of 7 — because a Python function call costs more than a queue append. This rung is at the top for what it SAYS, three lines that state exactly what depth means, and not for what it costs; the moment a tree can be deep, an iterative rung is the correct answer rather than the tidy one."
