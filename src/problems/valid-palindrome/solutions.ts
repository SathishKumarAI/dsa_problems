...
export const costWhy =
  "One walk of two pointers toward each other: each iteration advances i or retreats j, so together they cross the string once and the loop runs at most n times \u2014 that is the O(n). The skip loops inside do not change it, because a character skipped is a character the outer walk never revisits; the total number of pointer moves over the whole run is bounded by n, not by n per step. ON THE CLOCK THIS RUNG LOSES to the one below it: 9.4 ms against 8.1 on a 2·10⁵-character palindrome, best of 7. Both passes of the copy version run in C while this walk is a Python loop, and at this size that constant beats the extra allocation. What this rung keeps is the memory — O(1) against a second string the size of the input — and the early exit: a mismatch in the first two characters costs two comparisons here and a full copy there. The O(1) space is the two indices, which is what [[in-place|in place]] means here, and that is the entire argument for this rung over the cleaned-copy one: same time, none of the memory. The comparison itself folds case per character, which costs one operation and avoids a second pass over the input."
