// anagram-positions — the teaching document, as data.
//
// Converted from docs/deep/anagram-positions_explained.md by
...
export const doc: TeachingDoc = {
  ...
  understanding: `You are given a long piece of text and a short pattern, both lowercase letters. Slide a frame of
...
---`,
  ...
  arc: `Every rung here is an argument about **how much of the counting to redo**, and the argument only
...
---`,
  ...
  interview: `> **In an interview.** Write Approach 3 and state the two index facts as you write them: after
...
---`,
  scriptNote: `Every approach above, plus a test suite covering the statement's example, the overlapping-answers
...
all cross-checked against the sort-every-window oracle and against every other approach.`,
  script: `"""Where Every Anagram Hides - every approach in one file, plus a self-checking test suite.
