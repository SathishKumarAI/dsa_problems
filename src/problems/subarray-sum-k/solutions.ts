...
export const costWhy =
  "One pass over n elements. Each step does one addition to the running total, one map lookup for running \u2212 k and one map insert \u2014 all O(1) on average \u2014 so the time is O(n), about 2\u00b710\u2074 operations here against 2\u00b710\u2078 for the pairwise rungs. The O(n) space is the map of prefix sums, which in the worst case holds one entry per index because every running total is distinct. The seed is part of the correctness rather than the cost and belongs beside it: the map starts holding one occurrence of the sum 0, which is what lets a subarray starting at index 0 be counted."
