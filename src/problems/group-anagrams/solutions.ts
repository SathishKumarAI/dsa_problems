...
export const whyNow =
  "Sorting each word costs k log k just to make a label, and the label throws away nothing the counts do not already capture. A 26-slot tally is built in one pass over the word — linear in its length instead of linearithmic — and two anagrams produce the same tally by definition. MEASURED, THE SORTED KEY WINS HERE: 10.6 ms against 25.3 at n = 10⁴ words of length 20, best of 7. The bound is right and the constant is against it: the tally allocates 26 slots and joins them into a string per word, in Python, while sorting 20 characters runs in C. The tally overtakes as the word grows against the alphabet — at k = 20 over 26 letters it does not."
