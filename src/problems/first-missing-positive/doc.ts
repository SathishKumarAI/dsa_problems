// first-missing-positive — the teaching document, as data.
//
// Converted from docs/deep/first-missing-positive_explained.md by
...
export const doc: TeachingDoc = {
  ...
  understanding: `You are handed an unsorted array of integers. It may contain negatives, zeroes, duplicates, and
...
goes out of its way to tell you the input is expendable, it is telling you where to look.` },
  ...
  arc: `The principle every rung chases is *stop paying twice for what the constraints already told you*, and
...
---`,
  ...
  interview: `**Know cold: cyclic placement, and the boolean table.** This question exists to test exactly that
...
---`,
  scriptNote: `Every approach in one file, checked against all three examples from the statement, both smallest legal
...
1..n+1 always exists, so every legal input has one.`,
  script: `"""The Smallest Positive That Is Missing — every approach in one file, cross-checked.
