// window-maximum — the ladder: every way in, worst first.
//
...
export const approach = "Walk the array holding a deque of indices whose values are strictly decreasing. Before pushing the current index, pop from the back every index whose value is not greater than it — those can never be the maximum again, because the new value is larger and outlives them. Then drop the front if it has fallen out of the window. Once the first window is complete, the front of the deque is that window's maximum, every time. Each index enters and leaves once, so the whole thing is linear despite the nested-looking loop."
...
export const java = `public int[] maxSlidingWindow(int[] nums, int k) {
...
export const cpp = `vector<int> maxSlidingWindow(const vector<int>& nums, int k) {
    deque<int> best;
    vector<int> out;
