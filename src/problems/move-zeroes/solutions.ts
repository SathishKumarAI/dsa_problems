...
export const alternatives: Solution[] = [
  ...
  // every kept value is then swapped with itself.
  {
    ...
    costWhy:
      "O(n) time and O(1) space \u2014 the same bounds as the rung above it, which is exactly why this rung is about something other than cost. It swaps rather than writes, so it performs up to 2n memory writes where the write-index version performs one per surviving value — a difference in [[cache locality]] that is real and, here, NOT measurable: at the 10⁴ ceiling on an array half full of zeroes the two come out at 0.4 ms against 0.5, best of 7, with this rung marginally ahead. A tuple pack and unpack in CPython is cheap enough to hide the extra writes at this size. Take the write-index version for what it says rather than for a speed it does not buy — and note what just happened, because it is the lesson [[big-o|asymptotic]] notation keeps teaching: a consequence drawn FROM a bound is a claim, and a claim needs a clock.",
    whyNow:
