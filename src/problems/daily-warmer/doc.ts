// daily-warmer — the teaching document, as data.
//
// Converted from docs/deep/daily-warmer_explained.md by
...
export const doc: TeachingDoc = {
  problemId: "daily-warmer",
  understanding: `You are given a list of daily temperatures, one number per day, in calendar order. For each day you
...
and leaves once, so the total pop count is bounded by n no matter how the pops are distributed."**` },
  ...
  arc: `The principle every step here chases is *stop searching for the answer and let the answer come to
...
---`,
  ...
  interview: `**Memorise cold: the monotonic stack, and the amortised argument that justifies it.** The loop is six
...
---`,
  scriptNote: `All three approaches in one file, cross-checked on every case. Coverage: the statement's example; the
...
wider one over the full legal range.`,
  script: `"""Days Until Warmer - every approach in one file, cross-checked.
