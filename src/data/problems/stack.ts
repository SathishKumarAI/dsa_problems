...
export const stack: Problem[] = [
  {
    ...
    return not st`,
    walkthrough: [
      { text: 's = ( [ { } ] )\n\nstack: []', caption: "Openers push; closers must match the top." },
      { text: 'read ( → push\nread [ → push\nread { → push\n\nstack: ( [ {', caption: "Three openers stacked, newest on top." },
      { text: "read } → top is { ✓ pop\nread ] → top is [ ✓ pop\nread ) → top is ( ✓ pop\n\nstack: []", caption: "Each closer pops its exact partner — last opened, first closed." },
      { text: "end of string, stack empty → valid\n\ncounter-example \"(]\":\nread ( → push;  read ] → top is ( ✗ invalid", caption: "Empty stack at the end = balanced. Mismatch fails immediately." },
    ],
    alternatives: [
      {
  ...
  },
  {
    ...
    return answer`,
    walkthrough: [
      { cells: { values: [73, 74, 75, 71, 69, 72, 76, 73] }, caption: "For each day: how long until strictly warmer?" },
      { cells: { values: [73, 74, 75, 71, 69, 72, 76, 73], marks: { 0: "window", 1: "focus" } }, caption: "74 arrives: 73 on the stack is colder → pop it, answer[0] = 1." },
      { cells: { values: [73, 74, 75, 71, 69, 72, 76, 73], marks: { 2: "window", 3: "window", 4: "window" } }, caption: "After 75: stack [75]. Then 71, 69 stack up — decreasing order held." },
      { cells: { values: [73, 74, 75, 71, 69, 72, 76, 73], marks: { 3: "compare", 4: "compare", 5: "focus" } }, caption: "72 arrives: pops 69 (answer[4]=1) and 71 (answer[3]=2). Stack: [75, 72]." },
      { cells: { values: [73, 74, 75, 71, 69, 72, 76, 73], marks: { 2: "compare", 5: "compare", 6: "focus" } }, caption: "76 pops 72 (answer[5]=1) and 75 (answer[2]=4). Stack: [76]." },
      { cells: { values: [73, 74, 75, 71, 69, 72, 76, 73], marks: { 6: "done", 7: "done" } }, caption: "76 and final 73 never resolve → 0. Answers: [1,1,4,2,1,1,0,0]." },
    ],
    alternatives: [
      {
        name: "Brute force",
  ...
  },
  {
    ...
    examples: [
      { input: "heights = [2, 1, 5, 6, 2, 3]", output: "10", note: "Height 5 spanning the 5 and 6 bars." },
    ],
    hints: [
    ...
    return best`,
    walkthrough: [
      { cells: { values: [2, 1, 5, 6, 2, 3] }, caption: "Bars of a histogram. Find the largest rectangle under them." },
      { cells: { values: [2, 1, 5, 6, 2, 3], marks: { 0: "compare", 1: "focus" } }, caption: "1 < 2 → bar 0 is bounded. Area: 2 × 1 = 2. Pop it, push 1." },
      { cells: { values: [2, 1, 5, 6, 2, 3], marks: { 1: "window", 2: "window", 3: "window" } }, caption: "1, 5, 6 stack in increasing order — none bounded yet." },
      { cells: { values: [2, 1, 5, 6, 2, 3], marks: { 2: "compare", 3: "compare", 4: "focus" } }, caption: "2 arrives: pop 6 → area 6×1 = 6; pop 5 → area 5×2 = 10. New best." },
      { cells: { values: [2, 1, 5, 6, 2, 3], marks: { 1: "window", 4: "window", 5: "window" } }, caption: "Push 2, then 3. Sentinel 0 at the end will flush everything." },
      { cells: { values: [2, 1, 5, 6, 2, 3], marks: { 2: "done", 3: "done" } }, caption: "Flush: 3×1, 2×4, 1×6 — none beat 10. Answer 10 (height 5 over bars 2–3)." },
    ],
    alternatives: [
      {
        name: "Brute force",
