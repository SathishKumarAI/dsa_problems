...
export const problem: Problem = {
  ...
  costWhy:
    "O(n) time: the right edge advances exactly once per character, and the left edge only ever moves forward, so across the whole run the two edges make at most 2n moves between them \u2014 the inner while loop is not a nested loop for this reason ([[sliding window]]), which is the single most misread thing about window bounds. Each step does a constant amount of work: one set lookup, one insertion, one removal. The space is the set, which holds the characters currently inside the window: at most the size of the window, and at most the size of the alphabet, which is why the bound is O(min(n, alphabet)) rather than O(1) \u2014 this input is not restricted to 26 letters.",
