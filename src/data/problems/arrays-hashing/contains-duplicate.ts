...
export const problem: Problem = {
  ...
  costWhy:
    "One pass, and inside it one [[hash map|hash lookup]] and at most one insertion — both O(1) on average — so the time is linear in the number of elements EXAMINED, which the early exit often makes far fewer than n. The space is the set itself: up to n values when everything is distinct, which is the price of not sorting. Strictly the hash operations are O(1) expected, not worst case; adversarially chosen keys can [[collision|collide]] and degrade the pass, which is the one theoretical edge sorting keeps.",
