    ...
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-6">
      ...
      <header className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center gap-3">
          <h1 className="font-heading text-2xl font-semibold">{problem.title}</h1>
          <Badge variant="outline" className={cn("font-mono", difficultyClass[problem.difficulty])}>
            {problem.difficulty}
          </Badge>
        </div>
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-2">
          {problem.examples.map((ex, i) => (
            <div key={i} className="rounded-lg border bg-card p-3 font-mono text-sm">
              <div>
              ...
              {ex.note && <div className="mt-1 text-xs text-muted-foreground">{ex.note}</div>}
            </div>
          ))}
        </div>
      </section>
      ...
      <Tabs defaultValue="hints">
        <TabsList>
          ...
          {problem.walkthrough && <TabsTrigger value="walkthrough">Walkthrough</TabsTrigger>}
