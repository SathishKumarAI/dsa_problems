...
test("GET /api/problems lists every problem and names its journey", () => {
  ...
  assert.equal(list.find((p) => p.id === "single-number")?.journey, "single-number")
})

test("GET /api/journeys/:slug returns JSON-safe meta without functions", () => {
  const r = get("/api/journeys/two-sum")
  assert.equal(r.status, 200)
  const meta = r.body as { acts: { key: string }[]; presets: Record<string, unknown> }
...
test("POST run returns frames; bad input is a 400, unknown act a 404", () => {
  const r = post("/api/journeys/two-sum/run", { act: "hash", data: { nums: [2, 7, 11, 15], target: 9 } })
  assert.equal(r.status, 200)
  const { frames } = r.body as { frames: { note: string }[] }
  assert.ok(frames.length > 2)
  assert.ok(frames.every((f) => f.note))
  assert.equal(post("/api/journeys/two-sum/run", { act: "hash", data: { nums: "x" } }).status, 400)
  assert.equal(post("/api/journeys/two-sum/run", { act: "zzz", data: { nums: [1] } }).status, 404)
...
test("POST chart never includes acts past `upto` (no spoilers over the wire)", () => {
  ...
  const all = post("/api/journeys/two-sum/chart", { data }).body as { act: string }[]
  assert.deepEqual(all.map((r) => r.act), ["brute", "twoptr", "twopass", "hash"])
  const two = post("/api/journeys/two-sum/chart", { data, upto: 3 }).body as { act: string }[]
  assert.deepEqual(two.map((r) => r.act), ["brute", "twoptr"])
})

test("POST preset / parse / classify", () => {
  const p = post("/api/journeys/single-number/preset", { preset: "single" }).body as { data: { nums: number[] }; info: string }
  assert.equal(p.data.nums.length, 1)
  assert.ok(p.info)
  assert.equal(post("/api/journeys/single-number/preset", { preset: "nope" }).status, 404)
  const parsed = post("/api/journeys/two-sum/parse", { text: "1, 2 3", params: { target: "5" } }).body as { data: { nums: number[]; target: number } }
  assert.deepEqual(parsed.data, { nums: [1, 2, 3], target: 5 })
  assert.equal(post("/api/journeys/two-sum/parse", { text: "", params: {} }).status, 400)
  const v = post("/api/journeys/two-sum/classify", { data: { nums: [1, 2, 5, 11], target: 99 } }).body as { ok: boolean }
  assert.equal(v.ok, false)
})

test("algorithms: list, meta, run for arrays and graphs", () => {
  const list = get("/api/algorithms").body as { key: string }[]
  assert.ok(list.some((a) => a.key === "quick") && list.some((a) => a.key === "bfs"))
  const sort = post("/api/algorithms/quick/run", { array: [3, 1, 2] }).body as { frames: { arr: number[] }[] }
  assert.deepEqual(sort.frames.at(-1)!.arr, [1, 2, 3])
  const g = post("/api/algorithms/bfs/run", { n: 6 }).body as { graph: { nodes: unknown[] }; frames: unknown[] }
