// boats-to-save — the teaching document, as data.
//
// Converted from docs/deep/boats-to-save_explained.md by
...
export const doc: TeachingDoc = {
  ...
  understanding: `A crowd of people is waiting on a shore. Each of them has a weight. You have as many identical boats
...
---`,
  ...
  arc: `Every rung on this ladder is chasing one thing: **replace a question about groups with a question
...
---`,
  ...
  interview: `**Memorise cold — sort plus two converging pointers.** This is the expected answer and it is five
...
---`,
  scriptNote: `Every approach above, plus a test suite that runs the statement's example, the worked example, the
...
up to twelve people — its worst case is stated analytically above, not measured.`,
  script: `"""Fewest Boats for Everyone - every approach in one file, plus a self-checking test suite.
