...
import { HASH_MIN_BUCKETS, entriesOf, hashBuckets, hashLayout, hashSlot } from "./hashmap.ts"
...
test("layout chains colliding keys and counts hops to a present probe", () => {
  const m = hashLayout(entriesOf([[3, 0], [11, 1], [19, 2]]), { probe: 19, hit: true })
  assert.equal(m.buckets, HASH_MIN_BUCKETS)
  assert.equal(m.slot, 3)
  assert.deepEqual(m.chains[3].map((e) => e.key), [3, 11, 19])
...
test("a miss walks the whole chain; an empty bucket walks nothing", () => {
  const m = hashLayout(entriesOf([[3, 0], [11, 1]]), { probe: 27 })
...
test("hit and present are independent (count-scan case)", () => {
  const m = hashLayout(entriesOf([[5, 2]]), { probe: 5, hit: false, fmt: "times" })
  assert.equal(m.present, true)
