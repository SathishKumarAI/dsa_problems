...
export type ArrayStepType =
  "compare" | "swap" | "set" | "pivot" | "sorted" | "discard"
    ...
    *run(a) {
      ...
      for (let i = 0; i < n - 1; i++) {
        ...
        yield {
          ...
          indices: [n - 1 - i],
          line: 4,
          note: `largest of the pass bubbled to position ${n - 1 - i}`,
        }
      }
  ...
  insertion: {
    ...
    *run(a) {
      ...
      yield {
        ...
        indices: [0],
        line: 0,
        note: "first element is a sorted prefix",
      }
      for (let i = 1; i < n; i++) {
        const key = a[i]
        let j = i - 1
        yield {
          ...
          indices: [i],
          line: 1,
          note: `insert ${key} into sorted prefix`,
        }
        while (j >= 0 && a[j] > key) {
          yield {
            type: "compare",
        ...
        yield {
          ...
          values: [key],
          line: 4,
          note: `place ${key} at position ${j + 1}`,
        }
        yield {
          type: "sorted",
          indices: range(0, i),
          line: 4,
          note: `prefix a[0..${i}] sorted`,
        }
      }
    },
  },
      ...
      function* sort(lo: number, hi: number): Generator<ArrayStep> {
        ...
        for (let k = 0; k < merged.length; k++) {
          ...
          yield {
            ...
            values: [merged[k]],
            line: 4,
            note: `write ${merged[k]} to position ${lo + k}`,
          }
        }
      }
      yield* sort(0, n - 1)
    ...
    *run(a) {
      function* sort(lo: number, hi: number): Generator<ArrayStep> {
        ...
        yield {
          ...
          indices: [p, hi],
          line: 3,
          note: `pivot ${pivot} to position ${p}`,
        }
        yield {
          type: "sorted",
          indices: [p],
          line: 4,
          note: `pivot fixed at ${p}`,
        }
        yield* sort(lo, p - 1)
        yield* sort(p + 1, hi)
      }
    ...
    *run(a) {
        ...
        for (;;) {
          ...
          if (l <= end) {
            yield {
              ...
              indices: [l, big],
              line,
              note: `compare child ${a[l]} with ${a[big]}`,
            }
            if (a[l] > a[big]) big = l
          }
          if (r <= end) {
            yield {
              ...
              indices: [r, big],
              line,
              note: `compare child ${a[r]} with ${a[big]}`,
            }
            if (a[r] > a[big]) big = r
          }
          if (big === i) return
      ...
      for (let end = n - 1; end >= 1; end--) {
        ...
        yield {
          ...
          indices: [0, end],
          line: 2,
          note: `max ${a[end]} to position ${end}`,
        }
        yield {
          type: "sorted",
          indices: [end],
          line: 2,
          note: `position ${end} fixed`,
        }
  ...
  binary: {
    ...
    *run(a, target = 0) {
      ...
      yield {
        ...
        indices: [lo, hi],
        line: 0,
        note: `search for ${target} in a[${lo}..${hi}]`,
      }
      while (lo <= hi) {
        const mid = (lo + hi) >> 1
        yield {
          ...
          indices: [mid],
          line: 3,
          note: `mid = ${mid}: is a[mid] = ${a[mid]} equal to ${target}?`,
        }
        if (a[mid] === target) {
          yield {
            type: "sorted",
        ...
        if (a[mid] < target) {
          yield {
            type: "discard",
            indices: range(lo, mid),
            line: 4,
            note: `${a[mid]} < ${target} — target can't be in a[${lo}..${mid}], discard left half`,
          }
          lo = mid + 1
        } else {
          yield {
            type: "discard",
            indices: range(mid, hi),
            line: 5,
            note: `${a[mid]} > ${target} — target can't be in a[${mid}..${hi}], discard right half`,
          }
          hi = mid - 1
        }
        if (lo <= hi)
          yield {
            ...
            indices: [lo, hi],
            line: 1,
            note: `range narrowed to a[${lo}..${hi}]`,
          }
      }
      yield {
        type: "discard",
        indices: [],
        line: 6,
        note: `range empty — ${target} is not in the array`,
      }
    },
  },
  ...
  bfs: {
    ...
    *run(g) {
      ...
      yield {
        ...
        node: 0,
        line: 0,
        note: "start at node 0 — into the queue",
      }
      while (queue.length) {
        const u = queue.shift()!
        yield {
          ...
          node: u,
          line: 2,
          note: `visit ${u} — FIRST in, first out: the frontier expands in rings`,
        }
        for (const { v } of g.adj.get(u)!) {
          ...
          if (!seen.has(v)) {
            ...
            yield {
              ...
              edge: [u, v],
              line: 5,
              note: `${v} is new — queued. Distance from start: one ring further than ${u}`,
            }
          }
        }
      }
      yield {
        type: "done",
        line: 1,
        note: "queue empty — every reachable node visited, in distance order",
      }
    },
  },
  ...
  dfs: {
    ...
    *run(g) {
      ...
      yield {
        ...
        node: 0,
        line: 0,
        note: "start at node 0 — onto the stack",
      }
      while (stack.length) {
        const u = stack.pop()!
        yield {
          ...
          node: u,
          line: 2,
          note: `visit ${u} — LAST in, first out: dive deep before going wide`,
        }
        for (const { v } of g.adj.get(u)!) {
          yield {
            type: "edge",
          ...
          if (!seen.has(v)) {
            ...
            yield {
              ...
              edge: [u, v],
              line: 5,
              note: `${v} is new — stacked. It may be visited long before its siblings`,
            }
          }
        }
      }
      yield {
        type: "done",
        line: 1,
        note: "stack empty — one deep tendril at a time, that's the DFS shape",
      }
    },
  },
  ...
  dijkstra: {
    ...
    *run(g) {
      ...
      yield {
        ...
        dist: dist.slice(),
        line: 0,
        note: "dist[0] = 0; everyone else starts at ∞",
      }
      for (let round = 0; round < n; round++) {
        let u = -1
        for (let i = 0; i < n; i++)
          if (!done.has(i) && (u === -1 || dist[i] < dist[u])) u = i
        ...
        yield {
          ...
          dist: dist.slice(),
          line: 2,
          note: `lock in ${u} at distance ${dist[u]} — nothing unvisited can beat it`,
        }
        for (const { v, w } of g.adj.get(u)!) {
          ...
          if (dist[u] + w < dist[v]) {
            ...
            yield {
              ...
              dist: dist.slice(),
              line: 5,
              note: `relax! ${v} now reachable in ${dist[v]} via ${u}`,
            }
          }
        }
      }
      yield {
        ...
        dist: dist.slice(),
        line: 1,
        note: "every label is now the true shortest distance from node 0",
      }
    },
  },
}
...
export function buildArrayFrames(
  algo: ArrayAlgo,
  input: number[],
  target?: number
): ArrayFrame[] {
  const a =
    algo.kind === "search" ? input.slice().sort((x, y) => x - y) : input.slice()
  const frames: ArrayFrame[] = [
    {
      ...
      discard: [],
      line: -1,
      note: "press play — or step through",
      cmp: 0,
      swp: 0,
    },
  ]
...
export function buildGraphFrames(algo: GraphAlgo, g: Graph): GraphFrame[] {
  ...
  for (const s of algo.run(g)) {
    ...
    if (s.type === "relax" && s.node !== undefined && !visited.has(s.node))
      frontier.add(s.node)
    frames.push({
      visited: [...visited],
      frontier: [...frontier],
