...
const GENERIC = ["the problem", "code it", "the reveal", "start here", "your turn", "recap"]
const esc = (s: string) => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")

function actProse(j: AnyJourney, i: number): string[] {
  const a = j.acts[i]
  return [
    a.name, a.short, a.insight, a.idea, a.complexity,
    ...a.takeaways, ...(a.hints ?? []),
...
for (const j of JOURNEYS) {
  test(`${j.slug}: schema — every act has the fields the UI reads`, () => {
    ...
    for (const a of j.acts) {
      for (const k of ["key", "name", "short", "complexity", "idea"] as const) assert.ok(a[k], `${a.key}.${k}`)
      assert.ok(a.takeaways.length, `${a.key}.takeaways`)
      assert.ok(a.code.pseudo.length, `${a.key}.code.pseudo`)
      for (const q of a.quiz ?? []) {
        assert.ok(q.choices.length >= 2 && q.answer >= 0 && q.answer < q.choices.length, `${a.key} quiz shape`)
  ...
  test(`${j.slug}: code tabs match pseudocode line-for-line`, () => {
    for (const a of j.acts) {
      ...
      for (const lang of ["python", "java", "cpp"] as const) {
        ...
        if (lines) assert.equal(lines.length, n, `${a.key}.${lang} has ${lines.length} lines, pseudocode has ${n}`)
      }
    }
  })
  ...
  test(`${j.slug}: every generator drains on the sample and every frame narrates`, () => {
    for (const a of j.acts) {
      ...
      for (const f of frames) {
        assert.ok(typeof f.note === "string" && f.note.length > 0, `${a.key} frame without note`)
        if (f.predict) assert.ok(f.predict.choices.length >= 2 && f.predict.answer < f.predict.choices.length, `${a.key} predict shape`)
        const model = a.view(f as Frame, j.sample)
        assert.ok(model.panel.kind, `${a.key} view returns a panel`)
        if (f.line !== undefined && f.line >= 0) assert.ok(f.line < a.code.pseudo.length, `${a.key} line ${f.line} out of range`)
      }
      // frames must survive JSON — the API ships them over the wire
      assert.equal(JSON.stringify(frames), JSON.stringify(JSON.parse(JSON.stringify(frames))))
    }
  })
  ...
  test(`${j.slug}: progressive disclosure — no act names a later act`, () => {
    for (let i = 0; i < j.acts.length; i++) {
      const spoilers = j.acts.slice(i + 1).map((a) => a.name).filter((n) => !GENERIC.includes(n.toLowerCase()))
      const surfaces = [...actProse(j, i), ...drain(j.acts[i].run(j.sample, {})).flatMap((f) => [f.note, ...(f.predict ? [f.predict.q, ...f.predict.choices] : [])])]
      for (const s of spoilers) {
        const re = new RegExp(`\\b${esc(s)}\\b`, "i")
        for (const text of surfaces) assert.ok(!re.test(text), `act "${j.acts[i].name}" leaks "${s}": ${text.slice(0, 80)}`)
      }
    }
    // preset banners show from act 1 on, so they may not name anything past act 0
    const spoilers = j.acts.slice(1).map((a) => a.name).filter((n) => !GENERIC.includes(n.toLowerCase()))
    for (const [k, p] of Object.entries(j.presets))
      for (const s of spoilers) assert.ok(!new RegExp(`\\b${esc(s)}\\b`, "i").test(p.info ?? ""), `preset ${k} leaks "${s}"`)
  })
}

// ---------- correctness: every approach agrees with the contract ----------

const lastAnswer = <T>(frames: BaseFrame[]): T | undefined => (frames.findLast((f) => "answer" in f && f.answer !== undefined) as { answer?: T } | undefined)?.answer
...
test("two-sum: every approach returns the promised pair, including the traps", () => {
  ...
  for (const key of ["brute", "twoptr", "twopass", "hash"]) {
    const act = twoSum.acts.find((a) => a.key === key)!
    assert.equal(lastAnswer(drain(act.run({ nums: [1, 2, 5, 11], target: 99 }, {}))), undefined, key)
  }
})
...
test("two-sum: challenge act replays the learner's trace", () => {
  ...
  const frames = drain(act.run(data, { trace: { events: [{ op: "get", i: 0, v: 2 }, { op: "get", i: 1, v: 7 }], result: [0, 1], error: null } }))
  assert.equal(frames.length, 3)
  assert.deepEqual(lastAnswer(frames), [0, 1])
})

test("single-number: every approach finds the loner; XOR lies on two singles", () => {
  const cases = [[2, 2, 3], [4, 1, 2, 1, 2], [7], [5, 3, 5, 9, 9, 3, 12], [1, 1, 2, 2, 8]]
  for (const nums of cases) {
    ...
    for (const key of ["brute", "hash", "sort", "xor"]) {
      ...
      assert.equal(lastAnswer(drain(act.run({ nums }, {}))), want.single, `${key} on [${nums}]`)
    }
  }
  const broken = classifySingle([1, 2, 3, 3])
  assert.equal(broken.ok, false)
  const xor = singleNumber.acts.find((a) => a.key === "xor")!
  assert.equal(lastAnswer(drain(xor.run({ nums: [1, 2, 3, 3] }, {}))), 3, "1 ^ 2 = 3 — confidently wrong")
})
