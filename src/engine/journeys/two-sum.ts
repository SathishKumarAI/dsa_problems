...
function allPairs(nums: number[], target: number) {
  ...
  for (let i = 0; i < nums.length; i++)
    for (let j = i + 1; j < nums.length; j++) if (nums[i] + nums[j] === target) pairs.push([i, j])
  return pairs
}
...
function* runTwoPointer({ nums, target }: TwoSumData): Generator<F> {
  ...
  while (L < R) {
    ...
    const choices = ["left pointer → right, onto a bigger value", "right pointer → left, onto a smaller value", "both move inward"]
    if (sum < target) {
      L++
      yield {
        line: 5, order, s, L, R,
        predict: askedL ? undefined : { q: `${sum} is LESS than ${target}. Which pointer moves, and where?`, choices, answer: 0 },
      ...
      yield {
        line: 5, order, s, L, R,
        predict: askedR ? undefined : { q: `${sum} is MORE than ${target}. Which pointer moves, and where?`, choices, answer: 1 },
...
const eq = (a: number, b: number, sum: number, target: number): SumModel => ({ a, b, sum, target })

function needPanel(f: F, d: TwoSumData, label: string): StageModel["panel"] {
  const map = hashLayout(entriesOf(f.seen ?? []), { probe: f.need ?? null, hit: !!f.hit, label })
  return f.need !== undefined && f.i !== undefined
    ? { kind: "need", need: f.need, hit: !!f.hit, target: d.target, x: d.nums[f.i], map }
    : { kind: "hash", map }
}
...
const story: Act<TwoSumData, F> = {
  ...
  insight: "",
  tools: [{ name: "Array", role: "a shelf of prices, addressed by position. It's all you're handed — every approach below is a decision about what to build BESIDE it." }],
  ...
  code: { pseudo: ["given: nums and a target", "promise: exactly one pair sums to target", "you may not use the same element twice", "task: return the two indices"] },
  takeaways: [
  ...
  quiz: [
    { q: "What exactly must you return?", choices: ["the two values that hit the target", "the two indices where they sit", "the sum itself"], answer: 1, explain: "Values are easy to read off — the problem wants WHERE they are. Index bookkeeping shapes every solution." },
    { q: "How many valid pairs does the input promise?", choices: ["exactly one", "at least one", "any number"], answer: 0, explain: '"Exactly one solution exists" — the fast approaches quietly lean on that promise.' },
  ],
  run: runStory,
  view(f, d) {
    if (f.noChips) return { chips: null, panel: { kind: "story", glyph: "🎁 → 🛒 → ❓" } }
    return { chips: chipRow(d.nums, { focus: f.pair && !f.answer ? f.pair : [], answer: f.answer ?? [] }), panel: { kind: "none" } }
  },
}
...
const brute: Act<TwoSumData, F> = {
  ...
  tools: [{ name: "Array only", role: "read by index, nothing built alongside it. Zero extra memory is exactly why every question costs a full scan — you have nowhere to write an answer down." }],
  ...
  code: {
    pseudo: ["for i in 0..n-1:", "  for j in i+1..n-1:", "    if nums[i] + nums[j] == target:", "      return [i, j]", "no solution"],
    python: ["for i in range(len(nums)):", "    for j in range(i + 1, len(nums)):", "        if nums[i] + nums[j] == target:", "            return [i, j]", "return []"],
    java: ["for (int i = 0; i < nums.length; i++)", "    for (int j = i + 1; j < nums.length; j++)", "        if (nums[i] + nums[j] == target)", "            return new int[]{ i, j };", "return new int[]{};"],
    cpp: ["for (int i = 0; i < n - 1; i++)", "    for (int j = i + 1; j < n; j++)", "        if (nums[i] + nums[j] == target)", "            return { i, j };", "return {};"],
  },
  takeaways: [
  ...
  quiz: [
    { q: "Why does j start at i+1 instead of 0?", choices: ["it saves memory", "it halves the work AND bans using the same element twice", "it makes the loop O(n)"], answer: 1, explain: "Pairs are unordered — checking (i,j) and (j,i) is the same check. Starting at i+1 skips the mirror half and can never pair an element with itself." },
    { q: "The nested pair-checking loop costs…", choices: ["O(n) — one pass per element", "O(n log n) — like sorting", "O(n²) — every pair"], answer: 2, explain: "n choices for i times up to n for j ≈ n²/2 checks. Fine for a shelf, hopeless for a warehouse." },
  ],
  run: runBrute,
  view(f, d) {
    const dim = f.answer ? [] : range(f.i ?? 0)
    return {
      chips: chipRow(d.nums, { anchor: f.i !== undefined && !f.answer ? [f.i] : [], focus: f.j !== undefined && !f.answer ? [f.j] : [], dim, answer: f.answer ?? [] }),
      panel: f.sum !== undefined ? { kind: "sum", eq: eq(d.nums[f.i!], d.nums[f.j!], f.sum, d.target) } : { kind: "none" },
    }
  },
}
...
const twoptr: Act<TwoSumData, F> = {
  ...
  tools: [
    { name: "Sorted array", role: "ORDER is the tool here. Sorted, one comparison rules out a whole side of the range — unsorted, that deduction is illegal." },
    { name: "Index array", role: "the sort scrambles positions, so a parallel list of original indices rides along. The bookkeeping IS the O(n) space." },
    { name: "Two integer pointers", role: "L and R — two variables, not a structure. That's why the space bill stays small." },
  ],
  ...
  code: {
    pseudo: ["order = indices sorted by value", "L = 0, R = n-1", "while L < R:", "  s = val[L] + val[R]", "  if s == target: return their indices", "  s < target ? L += 1 : R -= 1"],
    python: ["order = sorted(range(len(nums)), key=lambda i: nums[i])", "L, R = 0, len(nums) - 1", "while L < R:", "    s = nums[order[L]] + nums[order[R]]", "    if s == target: return [order[L], order[R]]", "    L, R = (L + 1, R) if s < target else (L, R - 1)"],
    java: ["int[][] p = valueIndexPairs(nums); Arrays.sort(p, (a, b) -> a[0] - b[0]);", "int L = 0, R = p.length - 1;", "while (L < R) {", "    int s = p[L][0] + p[R][0];", "    if (s == target) return new int[]{ p[L][1], p[R][1] };", "    if (s < target) L++; else R--; }"],
    cpp: ["vector<array<int,2>> p; /* {value, index} */ sort(p.begin(), p.end());", "int L = 0, R = n - 1;", "while (L < R) {", "    int s = p[L][0] + p[R][0];", "    if (s == target) return { p[L][1], p[R][1] };", "    s < target ? L++ : R--; }"],
  },
  takeaways: [
  ...
  quiz: [
    { q: "Why must the array be sorted for two pointers to work?", choices: ["sorted arrays use less memory", "so one comparison can eliminate a whole pointer position", "duplicates disappear after sorting"], answer: 1, explain: "If the ends sum too small, the left value fails with its BEST partner — so it fails with all of them. Only order makes that deduction valid." },
    { q: "What does sorting break, and how do we repair it?", choices: ["nothing — return the sorted positions", "the original indices — carry each value's index along through the sort", "the values — copy the array first"], answer: 1, explain: "The answer must be ORIGINAL positions. Sort the indices by value (the tiny #numbers) or you return the wrong thing." },
  ],
  run: runTwoPointer,
  view(f, d) {
    const answer = f.answer ?? []
    const chips = chipRow(d.nums, { dim: range(d.nums.length).filter((k) => !answer.includes(k)), answer })
    ...
    return {
      chips,
      panel: { kind: "sorted", label: "sorted view (tiny number = original index)", chips: sorted, eq: f.sum !== undefined ? eq(f.s[L], f.s[R], f.sum, d.target) : undefined },
    }
  },
}
...
const twopass: Act<TwoSumData, F> = {
  ...
  tools: [
    { name: "Array", role: "unchanged and unsorted — two straight passes over it." },
    { name: "Hash Map", role: "value → index. Watch the panel: it is NOT a list of pills. It's a bucket table, and the hash decides which bucket a key lands in." },
  ],
  ...
  code: {
    pseudo: ["map = empty (value → index)", "for i, x in nums:   # pass 1", "  map[x] = i", "for i, x in nums:   # pass 2", "  j = map.get(target - x)", "  if j exists and j != i: return [i, j]"],
    python: ["seen = {}", "for i, x in enumerate(nums):", "    seen[x] = i", "for i, x in enumerate(nums):", "    j = seen.get(target - x)", "    if j is not None and j != i: return [i, j]"],
    java: ["Map<Integer, Integer> map = new HashMap<>();", "for (int i = 0; i < nums.length; i++)", "    map.put(nums[i], i);", "for (int i = 0; i < nums.length; i++) {", "    Integer j = map.get(target - nums[i]);", "    if (j != null && j != i) return new int[]{ i, j }; }"],
    cpp: ["unordered_map<int, int> numMap;", "for (int i = 0; i < n; i++)", "    numMap[nums[i]] = i;", "for (int i = 0; i < n; i++) {", "    int complement = target - nums[i];", "    if (numMap.count(complement) && numMap[complement] != i) return { i, numMap[complement] }; }"],
  },
  takeaways: [
  ...
  quiz: [
    { q: "In pass 2, why is the j != i guard mandatory?", choices: ["the map might point the current element at ITSELF when target = 2×x", "the map might be empty", "j could run past the end of the array"], answer: 0, explain: 'Pass 1 filed every element — including the one you\'re standing on. For target 6 at value 3, the map says "3 lives at your own index". The guard rejects that self-match.' },
  ],
  run: runTwoPassHash,
  view(f, d) {
    const answer = f.answer ?? []
    const dim = f.answer ? range(d.nums.length).filter((k) => !answer.includes(k)) : []
    const label = f.need === undefined ? "pass 1 — building the map" : "pass 2 — looking up complements"
    return { chips: chipRow(d.nums, { focus: f.i !== undefined && !f.answer ? [f.i] : [], dim, answer }), panel: needPanel(f, d, label) }
  },
}
...
const hash: Act<TwoSumData, F> = {
  ...
  tools: [
    ...
    { name: "Hash Map", role: "built AS you walk, so it only ever holds what's behind you. Below the waterline it's still buckets: hash, mod, chain — and it doubles itself when it gets crowded." },
  ],
  ...
  code: {
    pseudo: ["seen = empty map (value → index)", "for i, x in nums:", "  need = target - x", "  if need in seen: return [seen[need], i]", "  seen[x] = i"],
    python: ["seen = {}", "for i, x in enumerate(nums):", "    need = target - x", "    if need in seen: return [seen[need], i]", "    seen[x] = i"],
    java: ["Map<Integer, Integer> seen = new HashMap<>();", "for (int i = 0; i < nums.length; i++) {", "    int need = target - nums[i];", "    if (seen.containsKey(need)) return new int[]{ seen.get(need), i };", "    seen.put(nums[i], i); }"],
    cpp: ["unordered_map<int, int> numMap;", "for (int i = 0; i < n; i++) {", "    int complement = target - nums[i];", "    if (numMap.count(complement)) return { numMap[complement], i };", "    numMap[nums[i]] = i; }"],
  },
  takeaways: [
  ...
  quiz: [
    { q: "Why CHECK the map before STORING the current value?", choices: ["checking first is faster", "store first and x can match itself when target = 2×x", "the map must stay small"], answer: 1, explain: 'Store 3 first with target 6, then ask "seen 3?" — yes, yourself. Check-before-store means the map only ever holds elements BEHIND you.' },
    { q: "One-pass hash costs…", choices: ["O(n) time · O(n) space", "O(n log n) time · O(1) space", "O(n²) time · O(n) space"], answer: 0, explain: "One walk, O(1) lookups, and a map that can grow to n entries. Time is bought with space — that trade IS the pattern." },
  ],
  run: runHash,
  view(f, d) {
    const answer = f.answer ?? []
    const dim = f.answer ? range(d.nums.length).filter((k) => !answer.includes(k)) : range(f.i ?? 0)
    return {
      chips: chipRow(d.nums, { focus: f.i !== undefined && !f.answer ? [f.i] : [], dim, answer }),
...
export const TWO_SUM_CHALLENGE = {
  ...
  review: [
    { q: "No-solution path returns something sane (an empty array, not undefined)", check: (c: string) => /return\s*(\[\]|null)/.test(c) },
    { q: "One pass, not nested loops", check: (c: string) => (c.match(/\bfor\b|\bwhile\b/g) || []).length <= 1 },
    {
...
const challenge: Act<TwoSumData, F> = {
  ...
  tools: [{ name: "Your call", role: "array alone, sorted array + pointers, or a map. The structure you reach for IS the complexity you get — the scorecard will price your choice." }],
  ...
  *run({ nums, target }, ctx) {
    ...
    if (!events.length) yield { hold: 2, note: "your code never touched nums 🤨 — it returned without reading the array" }
    ...
    const ok = Array.isArray(r) && r.length === 2 && nums[r[0]] + nums[r[1]] === target
    yield {
      hold: 3,
  ...
  view(f, d) {
    return {
      chips: chipRow(d.nums, { focus: f.op === "get" ? [f.i!] : [], anchor: f.op === "set" ? [f.i!] : [], answer: f.answer ?? [] }),
...
const recap: Act<TwoSumData, F> = {
  ...
  tools: [
    { name: "Array", role: "the given. Fast by index, slow to search — that weakness is what every other structure was invented to fix." },
    { name: "Sorted array + two pointers", role: "buys the search with ORDER. Costs a sort up front and the index bookkeeping." },
    { name: "Hash Map", role: "buys the search with MEMORY. Costs O(n) space, a hash per key, and an occasional full rehash when the table doubles." },
  ],
  ...
  code: { pseudo: ["what transfers to the next problem:", "  1. always know the brute force", "  2. name its weakness out loud", "  3. ask: does order help? (two pointers)", "  4. ask: does memory help? (hash map)", "  5. return-what-exactly? read the contract"] },
  takeaways: [
  ...
  *run() {
  ...
  view(_f, d) {
    const rows = [brute, twoptr, twopass, hash].map((a) => ({ name: a.name, built: (a.tools ?? []).map((t) => t.name).join(" + "), cost: a.complexity, insight: a.insight }))
    return {
      chips: chipRow(d.nums, { answer: range(d.nums.length) }),
      panel: {
        kind: "recap",
        caption: "the journey, side by side — the middle column is the real story",
        rows,
        ...
        links: [
          { label: "Study the pattern: Two Pointers", detail: "the shape on its own — converge and chase, beyond this problem", href: "#/p/two-pointers" },
          { label: "Next problem: Single Number", detail: "a different superpower — can you beat the hash map's memory bill?", href: "#/journey/single-number" },
        ],
      },
    }
  },
...
export const twoSum: Journey<TwoSumData> = {
  slug: "two-sum",
  title: "Two Sum",
  subtitle: "one array, one target, four ways in — each earned by the last one's weakness",
  problemId: "pair-sum",
  leetcode: 1,
  ...
  presets: {
    ...
    nosolution: { label: "no solution (broken promise)", make: () => ({ nums: [1, 2, 5, 11], target: 99 }) },
    multi: { label: "two valid pairs (broken promise)", make: () => ({ nums: [1, 4, 2, 3], target: 5 }) },
  },
