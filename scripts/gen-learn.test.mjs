...
import { OUT_DIR, pages, renderProblem, runnableScript } from "./gen-learn.mjs"
...
test("a page shows every rung the problem page shows, in the same order", () => {
  for (const problem of PROBLEMS) {
    ...
    // "## Rung 3 — The one to remember". The prefix matters: a merged page also
    // carries the authored document's own "## Approach N" headings, and those
    // are a DIFFERENT ladder that is allowed to differ (see check-deep-drift).
    const headings = [...page.matchAll(/^## Rung (\d+) — (.+)$/gm)].map((m) => m[2])
    assert.deepEqual(
      headings,
      rungs.map((r) => r.name),
