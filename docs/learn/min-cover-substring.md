...
<!-- Generated by scripts/gen-learn.mjs from src/data/problems/sliding-window/min-cover-substring.ts
     and docs/deep/min-cover-substring_explained.md.
...
> The hardest window in the set, and the structure is what makes it manageable: grow the right edge until the window is valid, then shrink the left edge while it STAYS valid, recording the best as you shrink.
