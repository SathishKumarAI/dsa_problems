...
<!-- Generated by scripts/gen-learn.mjs from src/data/problems/binary-search/search-insert-position.ts
     and docs/deep/search-insert-position_explained.md.
...
> This is lower bound with a friendly name, and that is the takeaway: the answer is the first index whose value is at least the target, which is also the insertion point when the target is absent.
