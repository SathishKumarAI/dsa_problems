...
<!-- Generated by scripts/gen-learn.mjs from src/data/problems/trees/balanced-tree.ts
     and docs/deep/balanced-tree_explained.md.
...
> The condition has to hold at EVERY node, so the naive reading asks each node for its two subtree heights and then recurses — and measuring a height means walking that whole subtree, which happens again for every ancestor above it.
