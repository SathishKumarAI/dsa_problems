...
<!-- Generated by scripts/gen-learn.mjs from src/data/problems/linked-list/remove-nth-from-end.ts
     and docs/deep/remove-nth-from-end_explained.md.
...
## Rung 1 — Count, then walk forward
...
> A forward-only list cannot address anything from the end, so both rungs are doing the same conversion: turning nth from the back into a position measurable from the front.
