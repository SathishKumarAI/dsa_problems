...
<!-- Generated by scripts/gen-learn.mjs from src/data/problems/linked-list/swap-pairs.ts
     and docs/deep/swap-pairs_explained.md.
...
## Rung 2 — Array of nodes, then relink
...
## Rung 4 — Prev pointer, head special-cased
...
> The requirement is that the NODES move, and the bottom rung is the one that fails it: exchanging payloads is two lines, leaves every node where it was, and is wrong the moment a node carries more than an int or anything outside holds a pointer into the list.
