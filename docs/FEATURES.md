...
| Panel geometry, measured | `node test/panel-audit.mjs` walks all eight panel kinds on a real Chrome at 1536×864, picks each journey's LARGEST preset and its LAST act, and prints cells, overlapping pairs, cells outside the stage, smallest cell, and the smallest text on the stage. Asserts nothing — it is evidence for `docs/UX-AUDIT.md`. The densest case (the 80-cell DP table) is pinned in `test:ui` so it cannot regress. | `test/panel-audit.mjs`, `test/ui-smoke.test.mjs` | shipped (2026-09-09 run: 0 overlaps, 0 outside, min cell 40px, min text 12px) |
