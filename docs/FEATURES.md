...
| Static walkthrough player | Frames of cells (roles focus/compare/window/done + pointer labels); play (1.8 s/step), prev/next, restart, dot scrubber, ←/→ keys; terminal chrome + legend. Used by the **8 problems without a journey** (87 problems, 79 journeys). The ASCII `text` fallback it used to carry is **gone** — every problem that needed it now has a journey with a real tree, list or grid (B54 / V7), and `Frame.cells` is required rather than optional. | `step-player.tsx` | shipped |
