...
| Act stepper | Two shapes. **≥ xl** the ribbon: one button per unlocked act (`01`/✓ + name + short), current = mauve border, clicking switches act (pauses, clears predict/quiz/hints); while anything is locked, one dashed node `🔒 ? · n more · locked` — never a name; a newly revealed act zooms in (700 ms). **< xl** one 44 px row — `act 05 / 07 · One-Pass Hash ▾` with a lock icon while acts remain — opening the same ribbon, stacked with ↓ connectors, in a bottom sheet. | `act-stepper.tsx` | shipped (cdp: 1 node → 2 after reveal; UI test: the pill opens a sheet listing 7 acts at 390 px) |
...
| Bring three inputs | Every `edgeCases` entry: name, example (mono), `why`, `think`, and a **load this input** button that applies its preset (becomes **loaded ✓**, `aria-pressed`). Prose is technique-neutral so it may sit on act 0 (disclosure test covers it). Lives inside the problem panel. | `cards.tsx` `EdgeCaseList`, `use-journey.ts` `applyPreset` | shipped (cdp: click → preset `duplicates`, data `3, 1, 3, 8`, banner) |
