...
| Static walkthrough player | Frames with cells (roles focus/compare/window/done + pointer labels) or text; play (1.8 s/step), prev/next, restart, dot scrubber, ←/→ keys; terminal chrome + legend. Used by the **39 problems without a journey** (87 problems, 48 journeys). B54 deletes it: with the tree and list views proved, ASCII art is no longer the only thing those problems can show. | `step-player.tsx` | shipped (pre-existing) |
