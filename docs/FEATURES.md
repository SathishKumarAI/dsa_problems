...
| Static walkthrough player | Frames with cells (roles focus/compare/window/done + pointer labels) or text; play (1.8 s/step), prev/next, restart, dot scrubber, ←/→ keys; terminal chrome + legend. Used by the **19 problems without a journey** (87 problems, 68 journeys) — 11 of them still ASCII, and all eleven are the problems that carry Python only. B54 empties it one journey at a time: each new journey deletes its problem's hand-written frames, because `problems.test.ts` refuses to let both exist. | `step-player.tsx` | shipped (pre-existing) |
