...
| **PracHub** — https://prachub.com/positions/software-engineer | Coverage, and only coverage: ~6 000 questions candidates report being asked, tagged by company, which is a better answer to "is this problem actually asked" than any curated list of 150. Useful for deciding what to write next here | Crowdsourced recollections, so the statements are approximate and unverified — a question someone half-remembers a year later is not a specification. Freemium, with a Terms of Service. **Read it; never copy from it.** Everything in this repo is written in our own words and verified by running it, and pasted text would end both guarantees (`docs/deep/README.md`) |
