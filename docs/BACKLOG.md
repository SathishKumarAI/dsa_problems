...
| R6 | ☐ **One motion system** — a single easing and two durations (≈150 ms hover, 300–400 ms reveal) as tokens in `index.css`, and every ad-hoc `transition-*` in the features moved onto them. | `DESIGN.md` already carries a motion envelope; the code predates it in several places, so the app reads as several systems. | S | P2 |
