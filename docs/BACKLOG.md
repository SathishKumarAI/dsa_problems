...
| B31 | ☐ **One binary per block, not per case** — the runner compiles and launches once per (block, case), and Windows intermittently refuses a freshly written .exe (28 of 508 launches). Emitting a driver that loops over every case and prints one line each would cut compiles ~4x and remove the flake. | A gate that fails at random teaches people to ignore it. | S | B27 |
