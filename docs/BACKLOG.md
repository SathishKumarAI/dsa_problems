...
| R5 | ☐ **Elimination animation (Single Number)** — matching pairs cancel one at a time, the loner left pulsing and highlighted, the hint fading in under it. Reuses `use-flip.ts`; obeys the motion dial and reduced motion. | XOR's whole insight is "the pairs destroy each other". The act currently asserts it in prose. | S | P1 |
