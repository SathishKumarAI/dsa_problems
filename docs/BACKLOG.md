...
| B34 | ☐ **Modularise the data layer for fast edits** — `src/data/problems/*.ts` files are approaching 900 lines and hold prose, code in four languages and walkthrough frames in one literal. Split per problem (`problems/<pattern>/<id>.ts`) so one problem is one file, and a change is one small diff. Requested 2026-09-07. | Adding 50 problems to ten monolith files makes every edit a merge risk and every review a scroll. | M | B33 |
