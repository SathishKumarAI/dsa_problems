...
| R3 | ☐ **Problem panel that follows the learner** — the story act's Question / Hints / Edge cases accordion becomes a panel present on **every** act, collapsed by default from act 2 on, so "what was the input again?" costs one click instead of a walk back to act 1. Open sections persist through `lib/store.ts`. Hints stay as they are — visible up front because they are about *reading* the problem, with the 45 s idle offer separate. | Requested 2026-09-04. Today `HintList` and `EdgeCaseList` render only when `actIndex === 0`; from act 3 the problem statement is off screen entirely. Full spec: [`specs/2026-09-04-lesson-screen-redesign.md`](superpowers/specs/2026-09-04-lesson-screen-redesign.md). | M | **P1** |
| R4 | ☐ **Test-case drawer** — `DataControls` moves off-canvas right behind a flask toggle in the header; opening it **pushes** the main column (width transition, no overlay, no `fixed`) so nothing is covered. Invalid array text shows an inline error on Apply, never a silent no-op. Verify at 390 px and with the reading column open, where a push is a squeeze. | The controls sit under the stage today, so changing the input means scrolling away from the thing you are changing. | M | **P1** |
...
| R7 | ☐ **Keyboard and touch parity on the lesson screen** — arrows + enter through quiz options, `Esc` closes the drawer, and every hover affordance (sidebar row shift, quiz option scale) gets an `:active` equivalent. Extends F7's tap-to-peek rather than duplicating it. | Hover does not exist on touch, and the quiz is currently mouse-only. Accessibility basics, not polish. | S | P1 |
